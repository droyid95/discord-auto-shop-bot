...
def create_quickpay_url(config: BotConfig, invoice: PaymentInvoice, payment_type: str | None = None) -> str:
    try:
    ...
    payment = Quickpay(
        ...
        paymentType=payment_type or config.yoomoney_payment_type,
            ...
            for invoice in invoices:
                ...
                if operation.status == "success":
                    ...
                    if credited:
                        ...
                        await notify_user(bot, paid_invoice.user_id, paid_invoice.amount, "YooMoney")
...
async def notify_user(bot: commands.InteractionBot, user_id: int, amount: int, provider_name: str = "YooMoney") -> None:
    try:
        user = bot.get_user(user_id) or await bot.fetch_user(user_id)
        embed = disnake.Embed(
            title="Баланс пополнен",
            description=f"{provider_name} платеж найден и зачислен.",
            color=COLOR_PAYMENT,
        )
...
async def send_payment_log(
    ...
    try:
        ...
        if hasattr(channel, "send"):
            embed = disnake.Embed(title=provider_name, description="Пополнение баланса", color=COLOR_PAYMENT)
