...
class ShopCog(commands.Cog):
    ...
    async def create_deposit_panel(
        ...
        yoomoney_payment_type: str | None = None,
    ) -> None:
        ...
        print_payment_log(
            f"Deposit requested: user={inter.author.id} amount={amount} provider={provider} "
            f"payment_type={yoomoney_payment_type or '-'} promo={promo_code or '-'}"
        )
        ...
        try:
            if provider == "cryptopay":
                ...
                payment_type = yoomoney_payment_type or self.config.yoomoney_payment_type
                url = await asyncio.to_thread(create_quickpay_url, self.config, invoice, payment_type)
                method_title = "картой"
                view = PaymentLinkView(self, invoice.id, inter.author.id, url, f"Оплатить {method_title}")
                title = f"Пополнение {method_title}"
        ...
        await self.store.log(
            inter.author.id,
            f"{provider}_invoice_create",
            f"invoice={invoice.id} base_amount={invoice.base_amount} amount={invoice.amount} "
            f"payment_type={yoomoney_payment_type or '-'} promo={invoice.promo_code or '-'}",
        )
        await reply(
            field_embed(
                title,
...
class DepositModal(disnake.ui.Modal):
    def __init__(
        ...
        self.yoomoney_payment_type = yoomoney_payment_type
        components = [
        ...
        provider_title = method_title or ("CryptoPay" if provider == "cryptopay" else "YooMoney")
    ...
    async def callback(self, inter: disnake.ModalInteraction) -> None:
        ...
        try:
            ...
            await self.cog.create_deposit_panel(inter, amount, self.provider, promo_code, self.yoomoney_payment_type)
        except Exception as exc:
            await inter.edit_original_response(embed=error_embed(str(exc)), view=None)


class DepositMethodView(disnake.ui.View):
    def __init__(self, cog: ShopCog) -> None:
        super().__init__(timeout=120)
        self.cog = cog

    @disnake.ui.button(label="Картой", style=disnake.ButtonStyle.primary, emoji="🇷🇺")
    async def card(self, _: disnake.ui.Button, inter: disnake.MessageInteraction) -> None:
        if not self.cog.config.yoomoney_enabled and not self.cog.config.test_mode:
            await inter.response.send_message(embed=error_embed("Пополнение YooMoney сейчас отключено."), ephemeral=True)
            return
        await inter.response.send_modal(DepositModal(self.cog, "yoomoney", "SB", "картой"))

    @disnake.ui.button(label="Криптовалютой", style=disnake.ButtonStyle.secondary)
    async def crypto(self, _: disnake.ui.Button, inter: disnake.MessageInteraction) -> None:
        if not self.cog.config.cryptopay_enabled and not self.cog.config.test_mode:
            await inter.response.send_message(embed=error_embed("Пополнение CryptoPay сейчас отключено."), ephemeral=True)
            return
        await inter.response.send_modal(DepositModal(self.cog, "cryptopay", None, "CryptoPay"))
...
class CabinetSelect(disnake.ui.Select):
    def __init__(self, cog: ShopCog) -> None:
        ...
        if cog.config.yoomoney_enabled or cog.config.cryptopay_enabled or cog.config.test_mode:
            options.insert(0, disnake.SelectOption(label="Пополнить счет", value="deposit", description="Выбрать способ оплаты", emoji="💳"))
    ...
    async def callback(self, inter: disnake.MessageInteraction) -> None:
        if self.values[0] == "deposit":
            await inter.response.send_message(
                ...
                view=DepositMethodView(self.cog),
                ephemeral=True,
            )
            return
